...
var StatusInfo =
  ...
  (function () {
    ...
    const version = '0.3.8',
      ...
      style =
        'overflow: hidden; background-color: #fff; border: 1px solid #000; padding: 5px; border-radius: 5px;',
      buttonStyle =
        'background-color: #000; border: 1px solid #292929; border-radius: 3px; padding: 5px; color: #fff; text-align: center; float: right;',
      conditionStyle =
        'background-color: #fff; border: 1px solid #000; padding: 5px; border-radius: 5px;',
      conditionButtonStyle =
        'text-decoration: underline; background-color: #fff; color: #000; padding: 0',
        ...
        if (command === state[state_name].config.command) {
          switch (extracommand) {
            ...
            case 'config':
              ...
              if (args.length > 0) {
                if (args[0] === 'export' || args[0] === 'import') {
                  ...
                  if (args[0] === 'import') {
                    let json;
                    let config = msg.content.substring(
                      (
                        '!' +
            ...
            case 'config-conditions':
              ...
              if (condition === 'add') {
                ...
                if (state[state_name].conditions[condition.toLowerCase()]) {
                  sendConditionsConfigMenu(
                    'The condition `' + condition + '` already exists.'
                  );
                  return;
                }
              ...
              if (condition === 'remove') {
                ...
                if (!state[state_name].conditions[condition.toLowerCase()]) {
                  sendConditionsConfigMenu(
                    'The condition `' + condition + "` does't exist."
                  );
                  return;
                }
                ...
                sendConditionsConfigMenu(
                  'The condition `' + condition + '` is removed.'
                );
              }
              ...
              if (state[state_name].conditions[condition]) {
                if (args.length > 0) {
                  ...
                  if (
                    key === 'name' &&
                    value !== state[state_name].conditions[condition].name
                  ) {
                    state[state_name].conditions[value.toLowerCase()] =
                      state[state_name].conditions[condition];
            ...
            case 'toggle':
              ...
              if (!args.length) {
                makeAndSendMenu(
                  'No condition(s) were given. Use: <i>!' +
                    state[state_name].config.command +
                    ' ' +
                    extracommand +
                    ' prone</i>'
      ...
      handleConditions = (conditions, tokens, type = 'add', error = true) => {
        conditions.forEach((condition_key) => {
          if (!state[state_name].conditions[condition_key.toLowerCase()]) {
            if (error)
              makeAndSendMenu(
                'The condition `' + condition_key + '` does not exist.'
              );
            return;
          }
          ...
          tokens.forEach((token) => {
            ...
            let add =
              type === 'add'
                ? true
                : type === 'toggle'
                ? !token.get('status_' + getConditionByName(condition_key).icon)
                : false;
      ...
      handleStatusmarkerChange = (obj, prev) => {
        ...
        prev.statusmarkers =
          typeof prev.get === 'function'
            ? prev.get('statusmarkers')
            : prev.statusmarkers;

        if (
          state[state_name].config.showDescOnStatusChange &&
          typeof prev.statusmarkers === 'string'
        ) {
      ...
      getConditionByMarker = (marker) => {
        return (
          getObjects(state[state_name].conditions, 'icon', marker).shift() ||
          false
        );
      },
      ...
      sendConditionToChat = (condition, w) => {
        ...
        let icon = state[state_name].config.showIconInDescription
          ? getIcon(
              condition.icon,
              'margin-right: 5px; margin-top: 5px; display: inline-block;'
            ) || ''
          : '';
      ...
      getIcon = (icon, style = '') => {
        ...
        if (icon.includes('::')) {
          const tokenMarker = tokenMarkers.find(
            (marker) => marker.tag === icon
          );
          if (tokenMarker) {
            iconStyle += 'width: 24px; height: 24px;';
            iconStyle += style;
            return `<img style="${iconStyle}" src="${tokenMarker.url}"/>`;
          }
        }

        if (
          typeof icon_image_positions[icon] === 'undefined' &&
          !iconStyle.length
        )
          return false;
        ...
        if (Number.isInteger(icon_image_positions[icon])) {
          iconStyle +=
            'background-image: url(https://roll20.net/images/statussheet.png);';
          iconStyle += 'background-repeat: no-repeat;';
          iconStyle +=
            'background-position: -' + icon_image_positions[icon] + 'px 0;';
      ...
      sendConditionsConfigMenu = (message) => {
        if (
          ...
          typeof state[state_name].conditions === 'object'
        )
          setDefaults();
        ...
        for (let key in state[state_name].conditions) {
          ...
          listItems.push(
            '<span style="float: left;">' +
              getIcon(
                state[state_name].conditions[key].icon,
                'display: inline-block;'
              ) +
              state[state_name].conditions[key].name +
              '</span> ' +
              configButton
          );
          ...
          if (check && icons.includes(state[state_name].conditions[key].icon)) {
            message =
              message || '' + '<br>Multiple conditions use the same icon';
            check = false;
          }
        ...
        let backButton = makeButton(
          'Back',
          '!' + state[state_name].config.command + ' config',
      ...
      sendSingleConditionConfigMenu = (conditionKey, message) => {
        if (!conditionKey || !state[state_name].conditions[conditionKey]) {
          sendConditionsConfigMenu(
            'Condition ' + conditionKey + ' does not exist.'
          );
          return;
        }
        ...
        markers.forEach((marker) => {
          markerDropdown +=
            '|' + ucFirst(marker).replace(/-/g, ' ') + ',' + marker;
        });
        markerDropdown += '}';
        ...
        listItems.push(
          '<span style="float: left">Statusmarker: </span> ' + markerButton
        );
      ...
      sendMenu = (selected, show_names) => {
            ...
            if (token && token.get('statusmarkers') !== '') {
              ...
              statusmarkers.forEach((marker) => {
                ...
                if (
                  ...
                  if (con[0] && con[0].name)
                    active_conditions.push(con[0].name);
                }
              });
        ...
        for (let condition_key in state[state_name].conditions) {
          ...
          contents += makeButton(
            ...
            buttonStyle + 'float: none; margin-right: 5px;',
            condition.name
          );
        }
      ...
      sendConfigMenu = (first) => {
        ...
        let listItems = [
          ...
          '<span style="float: left">Show on Status Change:</span> ' +
            statusChangeButton,
          '<span style="float: left">Display icon in chat:</span> ' +
            showIconButton,
        ];
      ...
      makeAndSendMenu = (contents, title, settings) => {
        settings = settings || {};
        settings.whisper =
          typeof settings.whisper === 'undefined' || settings.whisper === 'gm'
            ? '/w gm '
            : '';
        title =
          title && title != ''
            ? makeTitle(title, settings.title_tag || '')
            : '';
        sendChat(
          script_name,
          settings.whisper +
      ...
      registerEventHandlers = () => {
        ...
        if (
          'undefined' !== typeof DeathTracker &&
          DeathTracker.ObserveTokenChange
        ) {
        ...
        if (
          'undefined' !== typeof InspirationTracker &&
          InspirationTracker.ObserveTokenChange
        ) {
        ...
        if (
          'undefined' !== typeof CombatTracker &&
          CombatTracker.ObserveTokenChange
        ) {
      ...
      setDefaults = (reset) => {
        ...
        if (!state[state_name].config) {
          ...
          if (!state[state_name].config.hasOwnProperty('sendOnlyToGM')) {
            state[state_name].config.sendOnlyToGM =
              defaults.config.sendOnlyToGM;
          }
          if (
            !state[state_name].config.hasOwnProperty('showDescOnStatusChange')
          ) {
            state[state_name].config.showDescOnStatusChange =
              defaults.config.showDescOnStatusChange;
          }
          if (
            !state[state_name].config.hasOwnProperty('showIconInDescription')
          ) {
            state[state_name].config.showIconInDescription =
              defaults.config.showIconInDescription;
          }
        }
