...
const ApplyDamage = (() => {
  ...
  const version = '1.2',
    ...
    handleError = (whisper, errorMsg) => {
      const output =
        `${whisper}<div style="border:1px solid black;background:#FFBABA;padding:3px">` +
        `<h4>Error</h4><p>${errorMsg}</p></div>`;
      sendChat('ApplyDamage', output);
    },
    finalApply = (results, dmg, type, bar, statusToken) => {
      ...
      Object.entries(results).forEach(([id, saved]) => {
        ...
        if (token && !saved) {
          if (boundedBar) {
            newValue = Math.min(
              Math.max(parseInt(token.get(barCur)) - dmg, 0),
              parseInt(token.get(barMax))
            );
          } else {
            newValue = parseInt(token.get(barCur)) - dmg;
          }
          if (statusToken) token.set(`status_${statusToken.getTag()}`, true);
        } else if (token && type === 'half') {
          if (boundedBar) {
            newValue = Math.min(
              Math.max(parseInt(token.get(barCur)) - Math.floor(dmg / 2), 0),
              parseInt(token.get(barMax))
            );
          } else {
      ...
      if (msg.type === 'api' && msg.content.search(/^!apply-damage\b/) !== -1) {
        const hasValue = ['ids', 'saves', 'DC', 'type', 'dmg', 'bar', 'status'],
          opts = Object.assign(
            {},
            defaultOpts,
            parseOpts(processInlinerolls(msg), hasValue)
          );
        ...
        let [statusToken] = libTokenMarkers.getStatuses(opts.status);
        if (opts.status && !statusToken) {
        ...
        finalApply(results, opts.dmg, opts.type, opts.bar, statusToken);
        const output = `${getWhisperPrefix(
          msg.playerid
          ...
          statusToken
            ? ` ${statusToken.getName()} status marker applied to tokens that failed the save.`
            : ''
        }</p></div>`;
    ...
    registerObserver = (event, observer) => {
      if (
        observer &&
        _.isFunction(observer) &&
        observers.hasOwnProperty(event)
      ) {
