class _Lighter extends ScriptBase({
  ...
  parser = new CommandParser('!lighter')
    .default((opts, msg) => {
  ...
  light = (opts, msg) => {
    ...
    if (
      target in this.state.lightTokens &&
      !this.state.lightTokens[target].lit
    ) {
    ...
    if (target in this.state.carriers) {
      ...
      if (!this.state.lightTokens[carrierState.carrying].lit) {
    ...
    this.state.lightTokens[lightToken.id] = {
      id: lightToken.id,
  ...
  drop = (opts, msg) => {
    ...
    if (target in this.state.carriers) {
      ...
      this.state.lightTokens[lightToken.id].carriedBy = null;
    }
  };
  ...
  pickup = (opts, msg) => {
    ...
    if (!(lightId in this.state.lightTokens)) return;
    ...
    this.state.lightTokens[lightId].carriedBy = carrierId;
  };
  ...
  showMenu = (who, playerid) => {
    ...
    Slo.whisper('Lighter', who, menu.toString(MENU_CSS));
  };

  handleTokenDelete = (obj) => {
    if (obj.id in this.state.lightTokens) {
      delete this.state.lightTokens[obj.id];
    }
  ...
  toggleLightSource = (lightToken) => {
    const lightState = this.state.lightTokens[lightToken.id];
    if (lightState.lit) {
      ...
      lightState.lit = true;
      const light = this.LIGHTS[lightState.type];
      lightToken.set({
        light_radius: light.radius,
        light_dimradius: light.dimradius,
        light_angle: 360,
